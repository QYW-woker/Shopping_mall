...
export default {
  ...
  methods: {
    ...
    changeActive(saleAttrValue, arr) {
      ...
      // 当点击时，将所有的ischecked改为false。只有点击选中的才为true
      // 遍历所有的售卖属性的ischecked值
      arr.forEach(item => {
        item.isChecked = '0'
      });
    ...
    // 加入购物车的回调
    async addShopCart() {
      // 1.发请求---将产品加入到服务器。派发action
      // 派发了actions也发了请求，如何判断加入购物车是成功还是失败了？
      try {
        await this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum });
        // 2. 服务器存储成功---进行路由跳转，传递参数
        // 如何将商品信息参数传递给跳转后的页面？，可以使用query参数；也可以存本地，直接取。
        // 简单数据可以使用query传递，复杂数据可以使用会话存储(不持久化，会话结束数据消失)
        // 本地存储或会话存储不能存对象；一般存的是字符串
        // stringify:将对象转JSON格式字符串
