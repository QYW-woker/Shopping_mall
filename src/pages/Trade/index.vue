  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ userDefaultAddress.fullAddress || '等待选择...' }}</span><br>
        收货人：<span>{{ userDefaultAddress.consignee || '等待选择...' }}</span>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      // 总价
      totalAmount: state => state.trade.orderInfo.totalAmount,
      // 商品种类数
      totalNum: state => state.trade.orderInfo.totalNum,
      // 订单号
    ...
    // 将来提交订单，最终地址
    userDefaultAddress() {
      // Array.find 查找数组当中，符合条件的元素。最终返回的是一个对象
  ...
  methods: {
    // 修改默认地址。
    // （排他方法：将所有的改为false，只有点击的那个改为true）
    ...
    async submitOrder() {
      ...
      let that = this.userDefaultAddress
      let data = {
        consignee: that.consignee,
        consigneeTel: that.phoneNum,
        deliveryAddre: that.fullAddress,
        paymentWay: 'ONLINE',
        orderComment: this.message,
        orderDetailList: this.orderInfo
      }
      // 直接在组件内发请求，不通过Vuex存储信息。将数据直接存储到组件的data身上
      let result = await this.$API.reqSubmitOrder(tradeNo, data)
      if (result.code == 200) {
        // 提交订单成功，存储订单号。给支付页面根据订单号支付
